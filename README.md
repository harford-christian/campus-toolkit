...
| Directory Search | `demos/facts-directory/` | Instant client-side search over a fabricated SIS export — students/guardians/teachers/classes, plus each student's schedule with the in-session class highlighted. Append `?sim=YYYY-MM-DD HH:MM` to time-travel (try `?sim=2026-09-11 14:45` for the reverse-order Friday) |
