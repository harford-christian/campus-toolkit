...
| Directory Search | `demos/facts-directory/` | Instant client-side search over a fabricated SIS export — students/guardians/teachers/classes, each student's schedule with the in-session class highlighted, an absent/late badge, emergency contacts and activities. Append `?sim=YYYY-MM-DD HH:MM` to time-travel (try `?sim=2026-09-11 14:45` for the reverse-order Friday). `node demos/facts-directory/verify.mjs` re-checks the dataset against the real app logic |
