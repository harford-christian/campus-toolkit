...
| Directory Search | `demos/facts-directory/` | Instant client-side search over a fabricated SIS export. Results group **Students → Teachers → Classes**; a teacher row leads with the class they are teaching *right now*. Each record carries the schedule in school-day order with the in-session class bolded, today's ABSENT / LATE / OUT badge, transportation (a recorded bus route vs an *assumed* car, split custody kept as two rows), siblings from Family ID, authorised pickup and emergency contacts. **Game-day dismissal** parsed from a coach's own calendar wording shows as a red bar (`Varsity Girls Soccer · 2:00 Dismissal`) and turns every class that dismissal cuts short the same red. **Favourites**: star students and the home screen lists them with today's status. **Room schedules** (home screen → building → room): every High School room as a tile with an *Open* / *In use* pill for right now, then that room's day with each period either its class or *Open*. A room hosting a class with no set period reads *Open?* and lists it rather than promising a free period (Room 603 in the demo). The two phone-heavy contact tabs load in a second call after first paint, so those sections read "Loading…" rather than a misleading "none on file". Append `?sim=YYYY-MM-DD HH:MM` to time-travel — `?sim=2026-09-11 14:45` for the reverse-order Friday (Period 1 meets at 2:37pm, schedule runs 8&rarr;1), `?sim=2026-09-08 11:50` to see the day bar name **Lunch** instead of reporting no class. Rebuild with `node tools/build-demo.mjs demos/facts-directory/build.json`; `node demos/facts-directory/verify.mjs` re-checks the dataset against the real app logic |
